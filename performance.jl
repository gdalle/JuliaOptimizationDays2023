...
md"""
## Toy example: GD for OLS
"""

# ╔═╡ 5092045a-b3be-473d-bfa5-d17fa36db1af
f(x, A, b) = sum(abs2, A * x - b)
...
function gradient_descent(
	f, ∇f, A, b;
	step=1e-3, iterations=100, return_history=false
)
	n, p = size(A)
	x = zeros(p)
	history = [x]
...
md"""
## Plot and test results
"""

# ╔═╡ 5c659445-3d2a-4aaf-8787-b07835b3c119
let
	history = gradient_descent(f, ∇f, A, b; return_history=true)
	Plots.plot(
		f.(history, Ref(A), Ref(b));
		xlabel="iteration", ylabel="squared error", label=nothing
	)
end

# ╔═╡ da3a3d5d-0a46-4fab-9cd2-148723005775
let
	x_init = gradient_descent(f, ∇f, A, b; iterations=0)
	x_final = gradient_descent(f, ∇f, A, b)
	Test.@test f(x_final, A, b) < f(x_init, A, b)
end
...
BenchmarkTools.@benchmark x .= x .+ y setup=(x=rand(3); y=rand(3))

# ╔═╡ ed4a5559-3338-4e45-82b3-7a2809a3e88f
md"""
The time to beat:
"""

# ╔═╡ beeea590-43f4-4f8d-bf12-417b76a86ea6
BenchmarkTools.@btime gradient_descent($f, $∇f, $A, $b);
...
ProfileCanvas.@profview gradient_descent(f, ∇f, A, b; iterations=10000)
...
ProfileCanvas.@profview_allocs gradient_descent(f, ∇f, A, b; iterations=10000)
...
function ∇f!(g, e, x, A, b)
	LinearAlgebra.mul!(e, A, x)
	e .-= b
	LinearAlgebra.mul!(g, A', e)
	g .*= 2
	return nothing
end

# ╔═╡ c61669e9-bbfa-4278-9303-107e3e01aa1e
function gradient_descent_fast(
	f::F, ∇f!::G, A, b;
	step=1e-3, iterations=100,
) where {F,G}
	...
	history_mat = Matrix{T}(undef, p, iterations + 1)
	history_mat[:, 1] .= x
	for t in 1:iterations
		∇f!(g, e, x, A, b)
		x .-= step .* g
...
# ╔═╡ 37744ac5-d5b1-483f-8054-ac773047d409
let
	history1 = gradient_descent(f, ∇f, A, b; return_history=true)
	history2_mat = gradient_descent_fast(f, ∇f!, A, b)
	p1 = Plots.plot(
		f.(history1, Ref(A), Ref(b));
		xlabel="iteration", ylabel="squared error", label=nothing, title="slow"
	)
	p2 = Plots.plot(
		f.(eachcol(history2_mat), Ref(A), Ref(b));
		xlabel="iteration", ylabel="squared error", label=nothing, title="fast"
	)
	Plots.plot(p1, p2)
end

# ╔═╡ f512ad5d-5c82-446b-a134-7696207431c8
let
	x = rand(p)
	g = rand(p)
	e = rand(n)
...
# ╔═╡ 779b8c31-4852-480b-802e-96fc84721974
begin
	history1 = gradient_descent(f, ∇f, A, b, return_history=true)
	history2_mat = gradient_descent_fast(f, ∇f!, A, b)
	Test.@test stack(history1) ≈ history2_mat
end

# ╔═╡ e1ebe968-059d-452f-ac33-8ff784e3747d
md"""
Now for the performance
"""

# ╔═╡ 8c492b39-065f-4c5f-b182-db8f6a1f2017
BenchmarkTools.@btime gradient_descent($f, $∇f, $A, $b);

# ╔═╡ f42eb5c9-d41d-47ef-9897-42f461dcc6e2
BenchmarkTools.@btime gradient_descent_fast($f, $∇f!, $A, $b);

# ╔═╡ 71597c24-6cfb-4d98-9a44-24b39fd16091
ProfileCanvas.@profview gradient_descent_fast(f, ∇f!, A, b; iterations=10000)
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "1222ec5591fee85ba52df1461a5fec998c0b64f0"
